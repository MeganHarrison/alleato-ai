...
interface NotionToD1SyncButtonProps {
  onSyncComplete?: () => void;
}

export function NotionToD1SyncButton({ onSyncComplete }: NotionToD1SyncButtonProps = {}) {
  ...
  const handleSync = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('/api/sync-notion-to-d1', {
        method: 'POST',
      });
      ...
      if (data.summary) {
        ...
        toast.success('Sync from Notion completed', {
  ...
  return (
    <>
      ...
      >
        ...
        {isLoading ? 'Syncing...' : 'Import from Notion'}
      </Button>

      <Dialog open={showResults} onOpenChange={setShowResults}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Notion to D1 Import Results</DialogTitle>
            <DialogDescription>
              Projects imported from Notion to D1 database
            </DialogDescription>
          </DialogHeader>
